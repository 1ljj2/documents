var vnoticeAudited = new Vue({
	...
	methods:{
        ...
        setTableTitle() {
            ...
            // 添加自定义slot-scope(操作栏)
            this.column.push(HeadActionSlot(true, 280));
            // 添加序号
       ...
       //-------------------查询表格--------------------------
        ...
        filter() {
            ...
            let url = this.noticePath + '/listAuditNotice';
            console.log("/listAuditNotice参数",data)
            CallAjaxPost(url, data, this.filterSuc);
            // 显示加载
            this.loading = true;
        },
        filterSuc(data) {
        	console.log("/listAuditNotice返回值",data.obj)
            // 取消显示加载
            this.loading = false;
        	//部门一二级展示
