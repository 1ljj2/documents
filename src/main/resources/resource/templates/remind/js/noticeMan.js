       ...
       //-------------------查询表格--------------------------
        ...
        filterSuc(data) {
        	...
        	for(let i =0;i<data.obj.list.length;i++){
        		...
        		this.message4Audit = data.obj.list[i].message4Audit;
        	}
        ...
        auditNoticeSuc(data){
            ...
            //检验是否需要发送短信提醒
            if (this.message4Audit == '1'){
                //---------需要发短信
                //根据userId查询用户的手机号码
                let data = this.userId;
                console.log("id：",data)
                let url = this.userPath + '/getPhoneByUserId';
                CallAjaxPost(url, data, this.getPhoneByUserIdSuc);
            }
        },
        getPhoneByUserIdSuc(data){
            console.log("用户手机号码：",data.obj)
            let messData = {
                phone: data.obj,
            };
        ...
        addNotice(){
        	...
        	if(this.fileList.length>0){
        		...
        		this.addFileNoticeSuc(1);
        	}
        },
        addFileNoticeSuc(data){
        	console.log('addFileNoticeSuc返回参数',data)
        	//新增前的对得到的数据进行校验
            if (this.checkNotice()) {
                return;
            }
        ...
        //------------附件上传--------------------------
        /**
         * 文件状态改变时的钩子，添加文件、上传成功和上传失败时都会被调用，更新文件列表和文件名信息
         * @param file 当前文件
         * @param fileList 文件列表
         */
