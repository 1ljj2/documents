var vMyArchive = new Vue({
        ...
        data: function () {
            return {
                ...
                //------------------------------------------------------------------------------------！！公有！！-------------------------
                allCourseId: [],//更据用户id获取相关的课程的id集合；
                categoryList: [],//分类
                termList: [],//学期
                courseList: [],//课程
                userList: [],//用户
                ...
                forReviewData: {processId: '', userId: '', stepId: '', archiveTemplateId: '',archiveName: '',archiveNumber:''},//选择流程，审核人的参数
        ...
        mounted() {
            this.getAllCourseId();//获取所有的和用户相关的课程id
        ...
        methods: {
            ...
            getArchiveDataSuc(data) {
                ...
                // }
                // else{
                //     for(let i=0; i<this.allCourseId.length;i++){
                //         for(let j=0;j<data.obj.list.length;j++){
                //             if (data.obj.list[j].courseId==this.allCourseId[i])
                //                 this.archiveData.push(data.obj.list[j])
                //         }
                //     }
                // }
             ...
             */
            openDrawer(o) {
                console.log("当前的状态：",o.archiveState)
                if (o.archiveState== null || o.archiveState=="" ) {
                //if (!o.processId) {//
                    this.submitAuditButton = true;

                    this.archive.archiveState="未审核"
                } else {

                    this.submitAuditButton = false;
                    if(o.archiveState=="B"){
                        this.archive.archiveState = "审核中"
                    }
                    else if(o.archiveState=="C"){
                        this.archive.archiveState = "审核通过"
                    }
                    // else if(o.archiveState=="A"){
                    //     this.archive.archiveState = "未审核"
                    // }
                    else if(o.archiveState=="D"){
                        this.submitAuditButton = true;
                        this.archive.archiveState = "审核不通过"
                    }
                ...
                this.forReviewData.archiveName= o.name;
                this.forReviewData.archiveNumber= o.number;
            ...
            downloadArchiveFileTem(index, row) {
                ...
                downloadFileEncrypt(row.archTemFileId);//下载错误
                downloadFileEncrypt(row.temFileId);
            },
            /**
             * 下载用户已经上传的文件
             * @param index
            ...
            openForReviewModal() {
                ...
                //     MessageWarning("未上传文件");
                //
                //     return
                // }
                // if ( this.fileList.length<this.archiveFilePoolData.length) {
                //     MessageWarning("文件未全部上传");
                //     return
                // }
                // //当前文件的标识
            ...
            forReview() {
                let data = {
                    ...
                    archiveName:this.forReviewData.archiveName,
                    archiveNumber:this.forReviewData.archiveNumber,
            ...
            getAllCourseId() {
                let url=this.archiveTemPath + "/listArchiveTemOfCourseId"
                let data = {
                   userId: localStorage.getItem("userId")
                }
                console.log("根据用户id",data)
                CallAjaxPost(url,data,this.getAllCourseIdSuc)
            },

            getAllCourseIdSuc(data) {
                console.log("根据用户id 获取到了courseId",data)
                this.allCourseId=data.obj;
            }
        }
    })
;