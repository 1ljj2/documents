...
//		String[] permitAllUrl = { "/test/hello", "/test/hello2", "/goods/listGoods" };
    ...
    private void configureTest(HttpSecurity http) throws Exception {
        System.out.println("执行到configureTest");
        http.cors() // 跨域
                // 关闭跨站拦截
                .and().csrf().disable()
        ...
        http.authorizeRequests()
                ...
                // 所有用户可访问
                .antMatchers("/**.docx","/**.pdf","/**.xlsx","/**.doc","/**.txt","/**.pptx","/**.png","/**.jpg").permitAll()
...
//                .addFilterBefore(new CheckEnvironmentFilter(), SecurityContextPersistenceFilter.class)
