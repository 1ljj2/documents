...
//            vo.setPhone(phone.substring(0, 3) + "****" + phone.substring(7, phone.length()));
    ...
    @Override
    public void wxinsert(User wxUser) {
        userMapper.wxinsert(wxUser);
    }

    @Override
    public PageInfoVo<UserInfo> selectAuditPeoplePageInfo(int pageNum, int pageSize) {
        new PageHelper().startPage(pageNum, pageSize);
        List<UserInfo> userRoleVoList = userMapper.listAuditUserList();
        PageInfo<UserInfo> pageInfo = new PageInfo<>(userRoleVoList);
         return new PageInfoVo<>(pageInfo);
    }

    @Override
    public void auditWxRegisterPeople(int id) {
        userMapper.auditWxRegisterPeople(id);
    }
