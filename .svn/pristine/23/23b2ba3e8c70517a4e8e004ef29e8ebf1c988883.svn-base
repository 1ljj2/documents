...
public interface UserMapper extends BaseMapper<User> {
	 ...
	 */
	Integer selectCountByUserName(@Param("name") String name);

	/**  2020/10/28/星期三 zhangjun
	 *
	  excel表格批量增加
	 */
//    void insertFromExlce(ExcelDataVO one);

	UserInfo selectUserInfo(@Param("openid") String openid);

    void wxinsert(User wxUser);

	List<UserInfo> listAuditUserList();

    void auditWxRegisterPeople(@Param("id") int id);
}