...
public class ArchiveTemplateEmpFileServiceImpl implements ArchiveTemplateEmpFileService {
    ...
    @Override
    public int insertList(List<ArchiveTemplateEmpFile> list) {
        return archiveTemplateEmpFileMapper.insertList(list);
    }
    ...
    @Override
    public PageInfoVo<ListArchiveFilePoolVo> getArchiveFilePool(Integer archiveTemId, Integer pageNum, Integer pageSize) {
        // 设置分页参数
        PageHelper.startPage(pageNum,pageSize);
        List<ListArchiveFilePoolVo> voList = archiveTemplateEmpFileMapper.getArchiveFilePool(archiveTemId);
        PageInfo<ListArchiveFilePoolVo> pageInfo = new PageInfo<>(voList);
        return new PageInfoVo<>(pageInfo);
    }
}
