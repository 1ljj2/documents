...
import org.jit.sose.mapper.NoticeFileConMapper;
...
public class NoticeServiceImpl implements NoticeService {
	...
	@Autowired
	private NoticeFileConMapper noticeFileConMapper;

	@Transactional
	@Override
	public PageInfoVo<ListNoticeVo> listNotice(ListNoticeParam param) {
		//设置过期公告seq为1
		noticeMapper.setOutTimeSeq();
		// 设置分页参数
	...
	public void addNotice(AddNoticeParam param) {
		...
		noticeFileConMapper.addNoticeFileCon(param);
	}
	...
	public void addAuditNoticeCon(AddNoticeAuditParam addNoticeAuditParam) {
		...
		noticeMapper.setNoticeInAudit(addNoticeAuditParam.getNoticeId());
	}
	...
	@Override
	public void checkAuditTrue(Integer id) {
		noticeMapper.checkAuditTrue(id);
	}

	@Transactional
	@Override
	public PageInfoVo<ListNoticeVo> listAuditNotice(ListNoticeParam param) {
		//设置过期公告seq为1
		noticeMapper.setOutTimeSeq();
		// 设置分页参数
        PageHelper.startPage(param.getPageNum(), param.getPageSize());
        //【标题、发布部门、发布用户、分页参数】
        List<ListNoticeVo> voList = noticeMapper.listAuditNotice(param);
        PageInfo<ListNoticeVo> pageInfo = new PageInfo<>(voList);
        return new PageInfoVo<>(pageInfo);
	}
	...
	public int newNoticeCount() {
		return noticeMapper.newNoticeCount();
	}

	@Transactional
	@Override
	public PageInfoVo<ListNoticeVo> listAuditNoticeMyRemind(ListNoticeParam param) {
		//设置过期公告seq为1
		noticeMapper.setOutTimeSeq();
		// 设置分页参数
        PageHelper.startPage(param.getPageNum(), param.getPageSize());
        //【分页参数】
        List<ListNoticeVo> voList = noticeMapper.listAuditNoticeMyRemind(param);
        PageInfo<ListNoticeVo> pageInfo = new PageInfo<>(voList);
        return new PageInfoVo<>(pageInfo);
	}
	...
	@Transactional
	@Override
	public int newNoticeToAuditCount(Integer userId) {
		return noticeMapper.newNoticeToAuditCount(userId);
	}
