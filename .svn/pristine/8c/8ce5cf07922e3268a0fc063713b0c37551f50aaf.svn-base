...
public class ProcessServiceImpl implements ProcessService {
    ...
    @Override
    public PageInfoVo<ListProcessForReviewVo> listForReviewByCondition(ListProcessParam param) {
        // 设置分页参数
        PageHelper.startPage(param.getPageNum(), param.getPageSize());
        // 查询集合(包括流程信息、所属学院idList、服务角色idList、公文模板idList)
        List<ListProcessForReviewVo> voList = processMapper.listForReviewByCondition(param);
        //赋初值给是否选中
        for (int i = 0; i < voList.size(); i++) {
            voList.get(i).setIsSelected(false);
        }
        PageInfo<ListProcessForReviewVo> pageInfo = new PageInfo<>(voList);
        return new PageInfoVo<>(pageInfo);
    }

    @Override
    public List<ListIdNameVo> listProcessByName(String processName) {
