...
var vRegister = new Vue({
    ...
    data: function () {
        return {
            ...
            userName: '', password: '', password1: '', phone: '', email: '',age: '',sex: '',code:'',
            msgCode: '', emailCode: '', picCode: '',//绑定属性
            imgSrc: METHOD_URL + firstPath + '/registerCode/getVerifyCode',
...
function checkAge() {
    if (vRegister.age === null || vRegister.age === '') {
        vRegister.ageErr = '请输入年龄！';
        return;
    }
    if (vRegister.age < 0 || vRegister.age > 100) {
        vRegister.ageErr = '年龄格式不正确';
        //vRegister.checkPasswordFlag1 = false;
    } else {
        vRegister.ageErr = '';
        //vRegister.checkPasswordFlag1 = true;
...
* */
function checkSex() {
    if (vRegister.sex === null || vRegister.sex === '') {
        vRegister.sexErr = '请输入性别！';
        return;
    }
   else {
        vRegister.sexErr = '';
...
* */
function checkCode() {
    if (vRegister.code === null || vRegister.code === '') {
        vRegister.codeErr = '请输入学号！';
        return;
    }
    else {
        vRegister.codeErr = '';
...
function comparePicCodeSuc(data) {
    ...
    if (data.obj === "codeExpired") {
        vRegister.picCodeErr = '验证码已失效，请重新输入！';
        vRegister.picCode = '';
        refreshRegister();//刷新验证码
        return;
    }
...
function sendMessageSus(data) {
    if (data.obj.success) {
        ...
        MessageWarning('短信发送失败');
        vRegister.msgCodeErr = data.obj.message;
    }
}
...
function refreshRegister() {
    let url = firstPath + "/registerCode/getVerifyCode?number=" + Math.random();
