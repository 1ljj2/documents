...
var vmyRemind = new Vue({
    ...
    data: function () {
        return {
            ...
            //-----------------------------人员审核部分--------------------------
            personPath:'/account/user',
            totalNum5: 0, pageNum5: 1, pageSize5: 4,
            peopleData: [],
            auditPerson:false,
            isAuditPersonMsg:{},
            //-------------------------------公告浏览部分-------------------------
            ...
            noticeAuditData: [],
            noticeAuditList: [],
            totalNum2: 0, pageNum2: 1, pageSize2: 4,  // 待审核公告分页参数
            // ---------------------TODO------------------文档审核待办部分----------------------------
            //新
            totalNum3: 0, pageNum3: 1, pageSize3: 4,
            ...
            // ----------------TODO---档案审核待办部分---------------------------------------------
            totalNum4: 0, pageNum4: 1, pageSize4: 4,
    ...
    methods: {
        ...
        checkPPermission(){
            let url = this.firstPath + '/checkAuditPersonPermission';
            CallAjaxGetNoParam(url, this.checkPPermissionSuc,null,null,false);
        },
        checkPPermissionSuc(data) {
            console.log("/checkPPermissionSuc返回值", data.obj)
            this.PP = data.obj;
        },
        //-------------------------------页面初始化加载项-------------------------
        initPage() {
            ...
            //人员审核
            if(this.PP == '1'){
                this.getPersonToAudit();
                setInterval(() =>
                    this.getPersonToAudit(), 3600000)//每1h调用一次getFileToAudit方法，进行数据更新
            }
       ...
       // ------------------------------------人员审核部分-------------------------------------------
        ...
        getPersonToAudit(){
            ...
            let url = this.personPath + '/selectAuditPeoplePageInfo';
            console.log("/selectAuditPeoplePageInfo参数", data)
            CallAjaxPost(url, data, this.getPersonToAuditSuc);
            // 显示加载
            this.loading = true;
        },

        getPersonToAuditSuc(data){
    	    console.log('selectAuditPeoplePageInfo待审核人员参数列表',data)
            this.peopleData=data.obj.list;
            this.totalNum5 = data.obj.total;
            // 再次设置当前页码,若查询记录为空，设为第一页
            this.pageNum5 = data.obj.pageNum === 0 ? 1 : data.obj.pageNum;
        },

        pass(obj){
    	   console.log(obj)
            this.auditPerson=true
            this.isAuditPersonMsg=obj
        },

        ensurePass(){
            let data = {
                id: this.isAuditPersonMsg.id,
            };
            let url = this.personPath + '/auditWxRegisterPeople';
            console.log("/auditWxRegisterPeople参数", data)
            CallAjaxPost(url, data, this.ensurePassSuc);
            // 显示加载
            this.loading = true;
           this.auditPerson = false
        },
        ensurePassSuc(data){
        ...
        openAuditFileModal(item) {
            ...
            //web端上传文件 覆盖问题
            this.auditFileInfo.reallyApplyUserId = item.applyUserId;
            //发请求获取模态框信息
        ...
        forReviewToNext() {
            ...
            if ((!this.isFileSignature)&&(this.auditFileInfo.isSign === 1)) {
                ...
                let data = {
                    ...
                    suffix:suffix,//文件名后缀
                    isSign:true,//文件名后缀
                    applyUserId: this.auditFileInfo.reallyApplyUserId,//申请人id,
                    applyFileId: this.auditFileInfo.fileId
                };
        ...
        previewArchive() {
           ...
           // CallAjaxPost(url,this.auditArchiveInfo.exampleId,this.previewArchiveSuc)
            CallAjaxPost(url,data,this.previewArchiveSuc)
