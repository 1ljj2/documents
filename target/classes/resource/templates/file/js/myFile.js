...
var vExcel = new Vue({
    ...
    data: function () {
        ...
        return {
            ...
            nowData: [],
            column: [
                {title: '文档名称', key: 'fileName',width: '150px'},
                {title: '学期', key: 'termName',width: '120px'},
                {title: '分类', key: 'categoryName',width: '120px'},
                {title: '备注', key: 'remark',width:'100px'},
                {title: '状态', key: 'auditStateString',width: '80px'},
            ...
            ShareMyFileModal: false, //分享文档模态框
            ShowMyPro: false, //点击看流程
            fileInfo: {
            ...
            //获取流程id
            getProcessIdByFileId: '',

            //分享文档
            //roleList: [],// 面向角色对象集合
            readUserList: [],//面向用户对象集合
            selectPermission: [],//分享权限集合
            allPermission: [
                {label: "A",
                    value: "下载权限"
                },
    ...
    methods: {
        ...
        initPage() {
            // 添加操作
            this.column.push(HeadActionSlot(true, 420));
            // 添加序号多选
        ...
        myFileSendMessageSuc() {
            ...
            this.filter();
            //this.auditMessage.selectedFileId.push(this.fileInfo.id);
            //this.auditMessage.selectProId.push(this.forReviewData.processId);
        },
        /**
         * 取消送审
         */
        ...
        * */
        ...
        openShareMyFileModal(index){
            this.ShareMyFileModal=true;
            this.sMess.fileId=this.nowData[index].fileId
        },
        ...
        * */
        ...
        showPro(row, column, data, event){   //根据文件id拿到流程id
           //  console.log(row)
           // console.log("==============");
            // console.log(this.auditMessage.selectedFileId);
            // console.log(this.auditMessage.selectProId);
            // console.log(this.auditMessage.auditAllMessage);

            // for (let i=0;i<this.auditMessage.selectedFileId.length;i++){
            //     if(this.auditMessage.selectedFileId[i]==row.fileId){

            if(data=='审核中'){
                console.log("点击文件的id是",row.fileId)
                let fileId=row.fileId;
                let data = fileId;
                console.log("参数",data);
                let url = this.fileAuditPath + '/getProcessIdByFileId';
                CallAjaxPost(url, data, this.returnShowPro);
            }
             // if(row.auditStateString=='审核中'){
             //     let fileId=row.fileId;
             //     let data = fileId;
             //     console.log("参数",data);
             //     let url = this.fileAuditPath + '/getProcessIdByFileId';
        ...
        //根据文件id拿到流程id之后 查询流程步骤
        returnShowPro(data){
            console.log("流程id是",data);
            this.auditMessage.selectProId=data.obj;
            this.viewProcessStep1();
        },
        ...
        viewProcessStep1() {
            console.log("来获取流程步骤信息")
            let data = this.auditMessage.selectProId;
            let url = this.processStepsPath + '/viewStepsByProcessId';
            //发同步请求，防止在点击【编辑】的时候出现还没有请求完成就显示出模态框
            CallAjaxPost(url, data, this.viewProcessStepSuc1);
        },

        //信息赋值给流程步骤条
        viewProcessStepSuc1(data) {
            console.log("viewProcessStepSuc11111111111111111111111111111:", data.obj)
            //将信息赋值给流程步骤条
            this.auditMessage.auditAllMessage = data.obj;
            this.ShowMyPro=true;
        },
        //关闭流程显示
        cancelShowPro(){
            this.ShowMyPro=false;
        },

        // test1(row, column, data, event){
        //     if(data=='审核中'){
        //         console.log("点击文件的id是",row.fileId)
        //     }
        //
        ...
        * */
        shareFile(){
             console.log("当前文件id",this.sMess.fileId)
             console.log("当前角色id",this.sMess.roleId);
             console.log("当前用户id",this.sMess.readUserId);
             console.log("当前权限",this.selectPermission);
             let data={
                 fileId:this.sMess.fileId,
                 roleIds:this.sMess.roleId,
                 userIds:this.sMess.readUserId,
                 downloadPermission: ((this.selectPermission.indexOf('A')>-1) ? 1 : 0),
                 updatePermission: ((this.selectPermission.indexOf('B')>-1) ? 1 : 0),
                 deletePermission: ((this.selectPermission.indexOf('C')>-1) ? 1 : 0),
             }
             let url=this.fileInfoPath+ '/shareFile';
            CallAjaxPost(url, data, this.shareFileLSuc);
        },
        ...
        getMyUserId(){
            this.shareMess.userId=localStorage.getItem("userId")
            console.log("当前登录的人的id",this.shareMess.userId)
        }
