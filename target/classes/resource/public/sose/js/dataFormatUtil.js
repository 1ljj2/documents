...
function CheckEmpty(value, msg) {
    if (value === undefined || value === null || value === "" || value.toString().trim().length === 0 || value.length === 0) {
        MessageWarning(msg);
        return true;
    } else {
        return false;
 ...
 * @returns {boolean}
 */
function CheckPhone(phone, nullMsg, wrongMsg) {
    let result = VERIFY_PHONE.test(phone);
    if (IsEmpty(phone)) {
        MessageWarning(nullMsg);
        return true;
    } else if (!result) {    //手机格式不正确
        MessageWarning(wrongMsg);
        return true;
    } else {   //手机格式正确
        return false;
    }
